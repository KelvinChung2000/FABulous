...
def create_project(project_dir: Path, lang: HDLType = HDLType.VERILOG) -> None:
    ...
    **This function will overwrite existing files in the target directory.**
    ...
    set_key(
        env_file,
        "FAB_MODELS_PACK",
        str(project_dir.absolute().resolve() / "Fabric" / f"models_pack.{new_suffix}"),
    )
...
class CommandPipeline:
    ...
    def execute_parallel(self) -> bool:
        ...
        The CLI instance (and bound methods) are not picklable which makes
        ProcessPoolExecutor unusable; use ThreadPoolExecutor to run
        `onecmd_plus_hooks` concurrently without serialization.
