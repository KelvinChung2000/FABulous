...
class Mux:
    ...
    def extendInputs(self, inputs: Iterable[GenericPort]) -> None:
        for i in inputs:
    ...
    def getFlattenMux(self) -> list[tuple[str, tuple[str, ...]]]:
        ...
        if isinstance(self.output, SharedPort):
            return list(zip(self.output.shareExpand(), groupedInputs, strict=False))
        return list(zip(self.output.expand(), groupedInputs, strict=False))
