...
def trim(number: float, precision: int) -> float:
    x = 10**precision
    temp = int(number * x)
    return temp / x
...
def generate_OpenLane_config(
    ...
    if tile.name in macroConfig:
        ...
        for configLine in macroConfig[default]:
            configText.append(f'"{configLine}": {macroConfig[default][configLine]},\n')
    configText.append('"FP_PDN_MULTILAYER": "false",\n')
    configText.append(f'"PL_TARGET_DENSITY_PCT": {density}\n')
    configText.append("}")

    cfg_path = Path(f"{openlaneDir}/designs/{tile.name}/config.json")
...
def generateCoreConfig(
    openlaneDir: str,
    coreName: str,
    tileHeightArea: float,
    tileWidthArea: float,
    _tiles: list[str],
    hooks: str,
    marginFABCore: float,
    ...
    for configLine in coreConfig:
        configText.append(f'"{configLine}": {coreConfig[configLine]},\n')
    configText.append(f'"PDN_MACRO_CONNECTIONS": {hooks},\n')
    configText.append(f'"PL_TARGET_DENSITY_PCT": {density},\n')
    configText.append(f"{tilePlacement}")
    configText.append("}")

    cfg_path = Path(f"{openlaneDir}/designs/{coreName}/config.json")
    with cfg_path.open("w") as f:
        f.writelines(configText)
...
def generateAutoConfig(configName: str) -> None:
...
def macroPlace(
    ...
    marginFabCore: float,
    marginTiles: float,
    fabricTileMap: list[list[str]],
    tilesToGenerateOL: list,  # list[TileOpenLane]
    tileHeights: list[int],
    tileWidths: list[int],
    _openlaneDir: str,
    _fabulousRunName: str,
) -> tuple[str, str]:
    ...
    superTileNames = {}
    macros = {}
    for _y, column in enumerate(fabricTileMap):
    ...
    for y, column in enumerate(fabricTileMap):
        ...
        for x, tile in enumerate(column):
            if x in superPositions:
                if (
                    ((y - 1 + superColumnSizes[x]) % superColumnSizes[x]) == 0
                    and y != len(fabricTileMap) - 1
                    and y != 0
                ):
                    superHeightPos = heightPos - tileHeights[y] * (
                        superColumnSizes[x] - 1
                    )
                    # record macro placement for super tiles
                ...
                macros[tile].append([x, y, widthPos, heightPos])
                macroHooks += (
                    f'"Tile_X{x}Y{y}_{tile} vccd1 vssd1 vccd1 vssd1,",'  # eFPGA_inst.
                )
    ...
    for macro in macros:
        ...
        for instances in macros[macro]:
            configTilePlacement += (
                f'        "Tile_X{instances[0]}Y{instances[1]}_{macro}"' + ": {\n"
            )
            configTilePlacement += (
                f'            "location": [{instances[2]}, {instances[3]}],\n'
            )
...
def ioPlace(
    tile: TileProtocol,
    combinDict: dict[str, dict[int, list[str]]],
    openlaneDir: str,
    prevPosGrid: dict[str, float] | dict,
    resize: str,
    ioSide: str,
    isSuperTile: bool,
    superTileCount: int,
    ...
    for side in combinDict:
        ...
        for layer in combinDict[side]:
            ...
            if resize == "Both":
                if side in ["Top", "Bottom"]:
                    ...
                    if isSuperTile:
                        distanceBetweenPins = trim(
                            (
                                (maxHeight - (superTileCount - 1) * tileOffset)
                                / (len(combinDict[side][layer]) + 1)
                            ),
                            4,
                        )
                    else:
            ...
            # new tile in following column
                ...
                # Super tiles and no resize handled above
            if isSuperTile and side in ["Left", "Right"]:
                superPartialHeight = tile.height / superTileCount
                superTileNonOffsetArea = [
                    [marginHorizontal, superPartialHeight - marginHorizontal]
                ]
                for i in range(superTileCount - 1):
                    superTileNonOffsetArea.append(
                        [
                            trim(
                                (
                                    (superPartialHeight + tileOffset) * (i + 1)
                                    + marginHorizontal
                                ),
                                4,
                            ),
            ...
            if side in ["Top", "Bottom"]:
                ...
                for _pin in combinDict[side][layer]:
                    posManufactureGrid.append(position)
                    position = trim((position + distanceBetweenPins), 4)
                # Snap all points
                snappedPositions = [
                    trim((round(x / gridSizeMet2) * gridSizeMet2 + marginVertical), 4)
                    for x in posManufactureGrid
                ]
            else:
                posManufactureGrid = []
                ...
                if isSuperTile:
                    ...
                    for pincount, _pin in enumerate(combinDict[side][layer]):
                        for index, area in enumerate(superTileNonOffsetArea):
                            if index == 0:
                                if (
                                    (pincount) % pinSubCount == 0
                                    and pincount != 0
                                    and pincount != (len(combinDict[side][layer]) - 1)
                                ):
                                    position = superTileNonOffsetArea[index + 1][0]
                                    posManufactureGrid.append(position)
                                    break
                                if position >= area[0] and position <= area[1]:
                                    posManufactureGrid.append(position)
                                    break
                            else:
                                if (
                                    position >= superTileNonOffsetArea[index - 1][1]
                                    and position <= area[0]
                                ):
                                    position = area[0]
                                    posManufactureGrid.append(position)
                ...
                # Snap all points
                snappedPositions = [
                    trim((round(x / gridSizeMet3) * gridSizeMet3 + marginHorizontal), 4)
                    for x in posManufactureGrid
                ]  #
    ...
    tcl_path = Path(f"{openlaneDir}/openlane/scripts/openroad/ioplacer.tcl")
...
def timingLoopsKill(name: str, openlaneDir: str) -> None:
    configText = [
    ...
    # generate macro_placement.cfg
    sdc_path = Path(f"{openlaneDir}/designs/{name}/{name}.sdc")
    with sdc_path.open("w") as f:
        f.writelines(configText)
