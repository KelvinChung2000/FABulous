...
class gdsGenerator:
    ...
    def __init__(
        ...
        if not Path(self.configPath).is_file():
            ...
            raise ValueError(
                "Configuration file for automated flow was not generated. It is now generated automatically. Please check the configuration file and rerun again."
            )
        ...
        if not config.has_section("OpenLaneMacro_DEFAULT"):
            raise ValueError(
                f"Define [OpenLaneMacro_DEFAULT] section in {self.configPath}"
            )
        ...
        if not config.has_option("General", "OpenLane_path"):
            raise ValueError(
                f"Define OpenLane_path in [General] section in {self.configPath}"
            )
        self.openlaneDir = config["General"]["OpenLane_path"]
        if not config.has_option("General", "PDKPath"):
            raise ValueError(
                f"Define PDKPath in [General] section in {self.configPath}, for demo RAM placement"
            )
        if not config.has_option("General", "TargetDensityStart"):
            logger.warning(
                f"You can define TargetDensityStart in [General] section in {self.configPath}"
            )
        else:
            self.targetDensity = float(config["General"]["TargetDensityStart"])
        if not config.has_option("General", "TerminateTilesStartHeight"):
            logger.warning(
                f"You can define TerminateTilesStartHeight in [General] section in {self.configPath}"
            )
        else:
            self.terminateTilesHeight = int(
                config["General"]["TerminateTilesStartHeight"]
            )
        ...
        for tileRow in self.fabricTileMap:
            for _tileColumn in tileRow:
                # X-axis corresponds to fabricDims[0]; Y-axis to fabricDims[1]
                fabricDims[0].append(self.standardTileSize)
                fabricDims[1].append(self.standardTileSize)
        fabricDims = (
            list(list(map(int, fabricDims[0]))),
            list(list(map(int, fabricDims[1]))),
        )
            ...
            for columnCount, tileColumn in enumerate(tileRow):
                ...
                if superTileName:
                    if superTileName not in tileNameBuffer:
                        tilesToGenerateOL.append(
                            tileOpenLane(
                                superTileName,
                                fabricDims[1][rowCount],
                                fabricDims[0][columnCount],
                                columnCount,
                                rowCount,
                                True,
                                self.getSubTileNames(superTileName),
                            )
                        )
        ...
        if config.has_option("General", "StarterTile"):
            ...
            logger.warning(
                "Starter Tile for resizing not defined. This could lead to a significant computing time increase. The most occuring tile will be taken."
            )
        ...
        for index, tile in enumerate(self.tilesToGenerateOL):
            if tile.name == self.starterTile:
                ...
                foundAt = index
        if starterTileFound:
            self.tilesToGenerateOL[0], self.tilesToGenerateOL[foundAt] = (
                self.tilesToGenerateOL[foundAt],
                self.tilesToGenerateOL[0],
            )
        else:
            logger.error(
                f"Starter tile {self.starterTile} does not exist. Check spelling."
            )
        ...
        if config.has_option("Dimensions", "StarterTileWidth"):
            self.tilesToGenerateOL[0].width = int(
                config["Dimensions"]["StarterTileWidth"]
            )
        if config.has_option("Dimensions", "StarterTileHeight"):
            self.tilesToGenerateOL[0].height = int(
                config["Dimensions"]["StarterTileHeight"]
            )
        tileFound = False
            ...
            if "_Width" in dim:
                ...
                for index, tile in enumerate(self.tilesToGenerateOL):
                    if tile.name == dimName:
                        ...
                        foundAt = index
                    if tileFound:
                        self.tilesToGenerateOL[foundAt].width = int(
        ...
        for tileConfig in tilesConfig:
            if "OpenLaneMacro_" in tileConfig:
                self.macroConfig[tileConfig.removeprefix("OpenLaneMacro_")] = config[
                    tileConfig
                ]
    ...
    def getSupertileClock(self, tileName: str) -> bool:
        clock = False
    ...
    def generate_pin_order_config(
        self, tileName: str, tileLocationX: int, tileLocationY: int, isSuperTile: bool
        ...
        if isSuperTile:
            ...
            # Frame Data
            for index, _clk in enumerate(withUserCLK):
            ...
            if externalBEL:
                withUserCLK = max(
                    externalBEL, key=attrgetter("withUserCLK")
                ).withUserCLK
            ...
            if (
                tileLocationY
                == max(self.tilesToGenerateOL, key=attrgetter("locationY")).locationY
                and externalPorts
                and tileLocationX
                != max(self.tilesToGenerateOL, key=attrgetter("locationY")).locationY
            ):
            ...
            if (
                tileLocationX
                == max(self.tilesToGenerateOL, key=attrgetter("locationX")).locationX
                and externalPorts
            ):
                for port in externalPorts:
    ...
    def resizeTile(
        self,
        tileID: int,
        tileEnlargePrevious: bool,
        tileEnlargeCurrent: bool,
        firstRun: bool,
        resize: str,
    ) -> None:  # indexTile, True, False, False, resizeDim
        ...
        if firstRun:
            ...
            if tileEnlargePrevious:
                # make tile larger
                if resize == "Both":
                    self.tilesToGenerateOL[tileID].height = (
                        self.tilesToGenerateOL[tileID].height + 1
                    )
                    self.tilesToGenerateOL[tileID].width = (
                        self.tilesToGenerateOL[tileID].width + 1
                    )
                elif resize == "Width":
                    self.tilesToGenerateOL[tileID].width = (
                        self.tilesToGenerateOL[tileID].width + 1
            ...
            if tileEnlargePrevious != tileEnlargeCurrent:
                ...
                if tileEnlargePrevious:
                    if resize == "Both":
                        self.tilesToGenerateOL[tileID].height = self.tilesToGenerateOL[
                            tileID
                        ].height
                        self.tilesToGenerateOL[tileID].width = self.tilesToGenerateOL[
                            tileID
                        ].width  # BIGMARK
                    elif resize == "Width":
                        self.tilesToGenerateOL[tileID].width = self.tilesToGenerateOL[
    ...
    def resizeTileOptFlow(
        self,
        indexTile: int,
        prevPinDistances: dict | list | str,
        resizeDim: str,
        ioPos: str,
        pinOrderChange: dict | list | str,
        cornerPos: str | bool,
        lastTermLoop: bool,
        ...
        logger.info("----------------------------------------------------")
        logger.info(
            "Generating tiles in OpenLane flow with tile resizing and start sizes defined in 'automated_config.ini'"
        )
        fixedOrderComplete = False
        tileEnlarge = False
        firstRun = True
        ...
        # removed unused densityManipulated
        ...
        generalFunctions.writeLog(
            ["Start resizing without optimizations \n"], logFile, "a"
        )
        ...
        pinSides, externUnused = self.generate_pin_order_config(
            self.tilesToGenerateOL[indexTile].name,
            self.tilesToGenerateOL[indexTile].locationX,
            self.tilesToGenerateOL[indexTile].locationY,
            self.tilesToGenerateOL[indexTile].isSuperTile,
        )
        initCombinDict = generalFunctions.initialRunCombinDict(
            synthesisFile,
            self.fabricGen.fabric.frameBitsPerRow,
            self.fabricGen.fabric.maxFramesPerCol,
            pinSides,
            self.tilesToGenerateOL[indexTile].isSuperTile,
        )
        combinDict, pinOrderChangeRun = generalFunctions.fixUpdateOrder(
            "",
            initCombinDict,
            pinOrderChange,
            resizeDim,
        ...
        while not fixedOrderComplete:
            generalFunctions.writeLog(
                [
                    f"Current Height:{self.tilesToGenerateOL[indexTile].height} ; Current Width:{self.tilesToGenerateOL[indexTile].width} ; Current Density: {currentTileDensity}\n "
                ],
                logFile,
                "a",
            )
            ...
            # config
            logger.info(
                f"Generate OpenLane config files for tile: {self.tilesToGenerateOL[indexTile].name}"
            )
            genConfig.generate_OpenLane_config(
                self.openlaneDir,
                self.tilesToGenerateOL[indexTile],
                self.macroConfig,
                True,
                currentTileDensity,
            )
            ...
            if resizeDim == "Height" and cornerPos:
                pinDistanceTemp, notEnoughPinArea = genConfig.ioPlace(
                    self.tilesToGenerateOL[indexTile],
                    combinDict,
                    self.openlaneDir,
                    prevPinDistances,
                    ...
                    cornerPos,
                    self.tilesToGenerateOL[indexTile].isSuperTile,
                    len(self.tilesToGenerateOL[indexTile].subTiles),
                    self.marginTiles,
                    self.gridSizeMet2,
                    self.gridSizeMet3,
                    self.marginMet2,
            ...
            if notEnoughPinArea:
                ...
                # check for errors
                tileEnlargeCurrent, densitySuggest = generalFunctions.check_flow(
                    self.openlaneDir, tileName, False, logger, self.ol_run_name
                )

            if densitySuggest:
                log_path = Path(
                    f"{self.openlaneDir}/designs/{tileName}/runs/{self.ol_run_name}/23-openroad-globalplacementskipio/openroad-globalplacementskipio.log"
                )
            ...
            if not tileEnlargeCurrent:
                generalFunctions.copyAndOverwrite(
                ...
                shutil.copyfile(
                    f"{self.openlaneDir}/openlane/scripts/openroad/ioplacer.tcl",
                    f"{self.openlaneDir}/designs/{tileName}/runs/{self.ol_run_name}_temp_success/ioplacer.tcl",
                )
            ...
            if not resizeDim:
                ...
                fixedOrderComplete, tileEnlarge, firstRun = self.resizeTile(
                    indexTile, tileEnlarge, tileEnlargeCurrent, firstRun, resizeDim
                )
            # if fixed order complete increase density and try to rerun (removed commented code)
        if resizeDim:
            generalFunctions.copyAndOverwrite(
                f"{self.openlaneDir}/designs/{tileName}/runs/{self.ol_run_name}_temp_success",
                f"{self.openlaneDir}/designs/{tileName}/runs/{self.ol_run_name}",
            ...
            shutil.copyfile(
                f"{self.openlaneDir}/openlane/scripts/openroad/ioplacer.tcl",
                f"{self.openlaneDir}/designs/{tileName}/runs/{self.ol_run_name}/ioplacer.tcl",
            )
        logger.info(f"Optimization free resizing for tile {tileName} finished")
        generalFunctions.writeLog(
            [
                f"Finished Height:{self.tilesToGenerateOL[indexTile].height} ; Finished Width:{self.tilesToGenerateOL[indexTile].width} ; Finished Density:{currentTileDensity}\n",
                "----------------------------------------------------------------------------------------------------- \n",
            ],
            logFile,
            "a",
        ...
        notSuccessRun = False
        logger.info(
            f"Start optimizing pinorder and netlist for {tileName} for {self.iterations} iterations"
        )
        generalFunctions.writeLog(
            ["Start resizing with optimizations \n"], logFile, "a"
        )
        ...
        if self.tilesToGenerateOL[indexTile].isSuperTile:
            pass  # superColumn was unused
        if resizeDim and not ioPos and self.optFlag:
            stats_dir = Path(f"{self.openlaneDir}/designs/{tileName}/statistics")
            ...
            for i in range(self.iterations):
                ...
                if not notSuccessRun:
                    ...
                    generalFunctions.writeLog(
                        [
                            f"Current Height:{self.tilesToGenerateOL[indexTile].height} ; "
                    ...
                    logger.info(
                        f"Current Height:{self.tilesToGenerateOL[indexTile].height} --- Current Width:{self.tilesToGenerateOL[indexTile].width} --- Current Density: {currentTileDensity} [Try: shrink]"
                    )

                    # move initial def and synthesis file
                    defFile = f"{self.openlaneDir}/designs/{tileName}/{tileName}.def"
                    synthesisFile = (
                        f"{self.openlaneDir}/designs/{tileName}/{tileName}.nl.v"
                    )
                    shutil.copyfile(
                        f"{self.openlaneDir}/designs/{tileName}/runs/{self.ol_run_name}/final/def/{tileName}.def",
                        defFile,
                    )
                    shutil.copyfile(
                        f"{self.openlaneDir}/designs/{tileName}/runs/{self.ol_run_name}/final/nl/{tileName}.nl.v",
                        synthesisFile,
                    )

                    portPairs, noConnection = generalFunctions.generate_pin_pairs(
                        synthesisFile, self.fabricGen, tileName
                    )

                    # netlist rearrangment
                    logger.info(f"start rearrange netlist for tile {tileName}")
                    nOpt = netListOpt.netListRearrange(
                        defFile,
                        synthesisFile,
                        f"{self.projectDir}/Tile/{tileName}/{tileName}_ConfigMem",
                    )
                    nOpt.plotIteration(
                        f"{self.openlaneDir}/designs/{tileName}/statistics/iteration_{i}"
                    )
                    ...
                    placement = pinOpt.placementRearrange(
                        defFile,
                        synthesisFile,
                        f"{self.openlaneDir}/designs/{tileName}",
                        pinSides,
                        portPairs,
                    )
                    ...
                    if notEnoughPinArea:
                        ...
                        logger.info(f"OpenLane flow for {tileName} complete.")
                        # check for errors
                        notSuccessRun, densitySuggest = generalFunctions.check_flow(
                            self.openlaneDir, tileName, False, logger, self.ol_run_name
                        )
                ...
                # re-run with same sizes because previous shrink run was unsuccessfull
                    ...
                    generalFunctions.writeLog(
                        [
                            f"Current Height:{self.tilesToGenerateOL[indexTile].height} ; Current Width:{self.tilesToGenerateOL[indexTile].width} ; Current Density:{currentTileDensity} [Try: re-order]\n"
                        ],
                        logFile,
                        "a",
                    )
    ...
    def propagateColumnInMap(
        self,
        columnIndex: int,
        initTileIndex: int,
        pinDistancePrevTile: dict,
        pinOrderChangePrevTile: dict,
        ioSide: str,
    ) -> tuple[list[str], dict]:
        fabricColumn = []
        generatedTiles = []
        ...
        if self.tilesToGenerateOL[initTileIndex].isSuperTile:
            for index, item in enumerate(fabricColumn[1:]):
                ...
                # part of super tile
                if (
                    self.getSupertileName(item) not in generatedTiles
                    and index != heightColumn
                ):
                    # search for tile
                    tileIndex: int
                    for index, tile in enumerate(self.tilesToGenerateOL):
                        if tile.name == self.getSupertileName(item):
                            tileIndex = index
                            break
                    self.resizeTileOptFlow(
                        tileIndex,
                        pinDistancePrevTile,
                        "",
                        ioSide,
                        pinOrderChangePrevTile,
                        "",
                        False,
                    )
                ...
                # Terminate Tile
                    ...
                    self.tilesToGenerateOL[tileIndex].width = self.tilesToGenerateOL[
                        initTileIndex
                    ].width
                    self.tilesToGenerateOL[tileIndex].height = self.terminateTilesHeight

                    # resize height but keep the width
                    ioSidetemp = ioSide or "Bottom"
                    pinDistanceTile, pinOrderChangeTile, tileDensity = (
                        self.resizeTileOptFlow(
                            tileIndex,
                            pinDistancePrevTile,
                            "Height",
        ...
        # CLB column
            ...
            for index, item in enumerate(fabricColumn[1:]):
                ...
                if item not in generatedTiles and index != heightColumn:
                    ...
                    self.tilesToGenerateOL[tileIndex].height = self.tilesToGenerateOL[
                        initTileIndex
                    ].height
                    self.resizeTileOptFlow(
                        tileIndex,
        ...
        if fabricColumn[0] not in generatedTiles:
            ...
            ioSidetemp = ioSide or "Top"
            pinDistanceTile, pinOrderChangeTile, tileDensity = self.resizeTileOptFlow(
                tileIndex,
                pinDistancePrevTile,
                "Height",
                ioSidetemp,
        ...
        # resized tile column checks (mixed columns logic removed)
    ...
    def run(self) -> None:
        ...
        if not self.coreOnly:
            ...
            for tile in self.tilesToGenerateOL:
                ...
                for _subdir, _dirs, files in os.walk(tile_dir):
                    for file in files:
                        if file[-2:] == ".v":
                            shutil.copyfile(
                                str(Path(_subdir) / file),
                                f"{self.openlaneDir}/designs/{tile.name}/src/{file}",
                            )
                shutil.copyfile(
                    str(Path(f"{self.projectDir}/Test") / "sequential_16bit_en_tb.v"),
                    f"{self.openlaneDir}/designs/{tile.name}/src/fabulous_tb.v",
                )
                shutil.copyfile(
                    str(Path(f"{self.projectDir}/Fabric") / "models_pack.v"),
                    f"{self.openlaneDir}/designs/{tile.name}/src/models_pack.v",
                )
                shutil.copyfile(
                    str(Path(f"{self.projectDir}") / "gate_map.v"),
                    f"{self.openlaneDir}/designs/{tile.name}/gate_map.v",
                )
            ...
            logger.info(
                f"Generating tiles in OpenLane flow with tile resizing and start sizes defined in {self.configPath}"
            )
            # run optimized resizing flow on starter tile
            pinDistanceInitialTile, pinOrderChangeInitialTile, tileDensity = (
                self.resizeTileOptFlow(0, "", "Both", "", "", "", False)
            )
            ...
            finishedTilesTemp, terminateTiles = self.propagateColumnInMap(
                tileColumn, 0, pinDistanceInitialTile, pinOrderChangeInitialTile, ""
            )
            ...
            for stepRight in range(tileColumn + 1, len(self.fabricTileMap[1])):
                ...
                for index, tile in enumerate(self.tilesToGenerateOL):
                    if (
                        tile.name == self.fabricTileMap[1][stepRight]
                        or self.fabricTileMap[1][stepRight] in tile.subTiles
                    ):
                        foundAt = index
                        break
                ...
                if self.tilesToGenerateOL[foundAt].name not in finishedTiles:
                    self.tilesToGenerateOL[foundAt].height = self.tilesToGenerateOL[
                        0
                    ].height
                    ...
                    if self.tilesToGenerateOL[foundAt].isSuperTile:
                        self.tilesToGenerateOL[foundAt].height = self.tilesToGenerateOL[
                            0
                        ].height * len(self.tilesToGenerateOL[foundAt].subTiles)
                # IO-Column
                if stepRight == (len(self.fabricTileMap[1]) - 1):
                    ...
                    finishedTilesTemp, unusedTerminateTiles = self.propagateColumnInMap(
                        stepRight, foundAt, pinDistanceTile, pinOrderChangeTile, "Right"
                    )
                ...
                # Normal or Super Column
                    ...
                    finishedTilesTemp, terminateTiles = self.propagateColumnInMap(
                        stepRight, foundAt, pinDistanceTile, pinOrderChangeTile, ""
                    )
                    finishedTiles.extend(finishedTilesTemp)
                    terminateTilesInfo.update(terminateTiles)

            # propagate columns to the left of the starter tile to find min width of other columns
            for stepLeft in reversed(range(tileColumn)):
                # search for tile
                foundAt: int
                for index, tile in enumerate(self.tilesToGenerateOL):
                    if (
                        tile.name == self.fabricTileMap[1][stepLeft]
                        or self.fabricTileMap[1][stepLeft] in tile.subTiles
                    ):
                        foundAt = index
                        break
                # change height to init tile
                ...
                if stepLeft == 0:
                    ...
                    finishedTilesTemp, unusedTerminateTiles = self.propagateColumnInMap(
                        stepLeft, foundAt, pinDistanceTile, pinOrderChangeTile, "Left"
                    )
                    finishedTiles.extend(finishedTilesTemp)
                    terminateTilesInfo.update(unusedTerminateTiles)
                # Normal or Super Column
                    ...
                    finishedTilesTemp, terminateTiles = self.propagateColumnInMap(
                        stepLeft, foundAt, pinDistanceTile, pinOrderChangeTile, ""
                    )
                    finishedTiles.extend(finishedTilesTemp)
                    terminateTilesInfo.update(terminateTiles)
            ...
            for tileIndex in terminateNorthTilesIndexes:
                if tileIndex not in completedNorthTilesIndexes:
                    ...
                    if self.getSupertileName(underTile):
                        isSuperColumn = True
                    if (
                        self.tilesToGenerateOL[tileIndex].name
                        == self.fabricTileMap[0][0]
                        or self.tilesToGenerateOL[tileIndex].name
                        == self.fabricTileMap[0][-1]
                    ):
                        isSuperColumn = True

                    self.resizeTileOptFlow(
                        tileIndex,
                        terminateTilesInfo[self.tilesToGenerateOL[tileIndex].name][0],
                        "",
                        iopos,
                        terminateTilesInfo[self.tilesToGenerateOL[tileIndex].name][1],
                        cornerpos,
                        isSuperColumn,
                    )
            ...
            for tileIndex in terminateSouthTilesIndexes:
                if tileIndex not in completedSouthTilesIndexes:
                    ...
                    if (
                        self.tilesToGenerateOL[tileIndex].name
                        == self.fabricTileMap[-1][0]
                        or self.tilesToGenerateOL[tileIndex].name
                        == self.fabricTileMap[-1][-1]
                    ):
                        isSuperColumn = True
        ...
        tcl_path = Path(f"{self.openlaneDir}/openlane/scripts/openroad/ioplacer.tcl")
        ...
        proc = sp.Popen(
            fabricFolder, shell=True, stdin=sp.PIPE, stdout=sp.PIPE, encoding="utf8"
        )
        proc.communicate()
        if proc.stdout is not None:
            proc.stdout.close()
        ...
        base = Path(f"{self.openlaneDir}/designs/{self.fabulousRunName}")
        for p in [
            base / "macros",
            base / "macros/gds",
            base / "macros/lef",
            base / "macros/lib",
            base / "macros/multicorner",
            base / "macros/multicorner/min",
            base / "macros/multicorner/nom",
            base / "macros/multicorner/max",
            base / "src/BB",
        ]:
            with suppress(OSError):
        ...
        for tile in self.tilesToGenerateOL:
            if tile.name != "NULL":
                shutil.copyfile(
                    f"{self.openlaneDir}/designs/{tile.name}/runs/{self.ol_run_name}/final/nl/{tile.name}.nl.v",
                    f"{self.openlaneDir}/designs/{self.fabulousRunName}/src/BB/{tile.name}.v",
                )
                shutil.copyfile(
                    f"{self.openlaneDir}/designs/{tile.name}/runs/{self.ol_run_name}/final/gds/{tile.name}.gds",
                    f"{self.openlaneDir}/designs/{self.fabulousRunName}/macros/gds/{tile.name}.gds",
                )
                shutil.copyfile(
                    f"{self.openlaneDir}/designs/{tile.name}/runs/{self.ol_run_name}/final/lef/{tile.name}.lef",
                    f"{self.openlaneDir}/designs/{self.fabulousRunName}/macros/lef/{tile.name}.lef",
                )
                shutil.copyfile(
                    f"{self.openlaneDir}/designs/{tile.name}/runs/{self.ol_run_name}/final/lib/nom_ss_100C_1v60/{tile.name}__nom_ss_100C_1v60.lib",
                    f"{self.openlaneDir}/designs/{self.fabulousRunName}/macros/lib/{tile.name}.lib",
                )
                shutil.copyfile(
                    f"{self.openlaneDir}/designs/{tile.name}/runs/{self.ol_run_name}/final/spef/max/{tile.name}.max.spef",
                    f"{self.openlaneDir}/designs/{self.fabulousRunName}/macros/multicorner/max/{tile.name}.spef",
                )
                shutil.copyfile(
                    f"{self.openlaneDir}/designs/{tile.name}/runs/{self.ol_run_name}/final/spef/nom/{tile.name}.nom.spef",
                    f"{self.openlaneDir}/designs/{self.fabulousRunName}/macros/multicorner/nom/{tile.name}.spef",
                )
                shutil.copyfile(
                    f"{self.openlaneDir}/designs/{tile.name}/runs/{self.ol_run_name}/final/spef/min/{tile.name}.min.spef",
                    f"{self.openlaneDir}/designs/{self.fabulousRunName}/macros/multicorner/min/{tile.name}.spef",
                )
        if self.ramGenerate:
            shutil.copyfile(
                f"{self.openlaneDir}/designs/{self.ramName}/runs/{self.ol_run_name}/final/nl/{self.ramName}.nl.v",
                f"{self.openlaneDir}/designs/{self.fabulousRunName}/src/BB/{self.ramName}.v",
            )
            shutil.copyfile(
                f"{self.openlaneDir}/designs/{self.ramName}/runs/{self.ol_run_name}/final/gds/{self.ramName}.gds",
                f"{self.openlaneDir}/designs/{self.fabulousRunName}/macros/gds/{self.ramName}.gds",
            )
            shutil.copyfile(
                f"{self.openlaneDir}/designs/{self.ramName}/runs/{self.ol_run_name}/final/lef/{self.ramName}.lef",
                f"{self.openlaneDir}/designs/{self.fabulousRunName}/macros/lef/{self.ramName}.lef",
            )
            shutil.copyfile(
                f"{self.openlaneDir}/designs/{self.ramName}/runs/{self.ol_run_name}/final/lib/nom_ss_100C_1v60/{self.ramName}__nom_ss_100C_1v60.lib",
                f"{self.openlaneDir}/designs/{self.fabulousRunName}/macros/lib/{self.ramName}.lib",
            )

        # move eFPGA fabric verilog files
        for _subdir, _dirs, files in os.walk(f"{self.projectDir}/Fabric"):
            for file in files:
                if file != "BlockRAM_1KB.v":
                    shutil.copyfile(
                        str(Path(f"{self.projectDir}/Fabric") / file),
                        f"{self.openlaneDir}/designs/{self.fabulousRunName}/src/{file}",
                    )
        ...
        tileWidths = []
        for _columnNum, tile in enumerate(self.fabricTileMap[1]):
            # search for tile
            foundAt: int = 0
            for index, tileSearch in enumerate(self.tilesToGenerateOL):
                if tileSearch.name == tile or tile in tileSearch.subTiles:
        ...
        heightPos = (
            fabricTileHeightTotal
            + self.marginFab
            + self.marginFabCore
            + (rowNum - 1) * self.marginTiles
        )
        configTilePlacement, macroHooks = genConfig.macroPlace(
            heightPos,
            self.marginFab,
            self.marginFabCore,
            self.marginTiles,
            self.fabricTileMap,
            self.tilesToGenerateOL,
            tileHeights,
            tileWidths,
            self.openlaneDir,
            self.fabulousRunName,
        )
        # RAM placement

        if self.ramGenerate:
            ramBlocksCount = int(
                fabricTileHeightTotal / (self.ramHeight + self.marginRAM)
            )
            heightPos = (
                fabricTileHeightTotal
                + self.marginFab
        ...
        logger.info(
            f"Generate OpenLane core config files for core: {self.fabulousRunName}"
        )
        genConfig.generateCoreConfig(
            self.openlaneDir,
            self.fabulousRunName,
            fabricTileHeightTotal,
            fabricTileWidthTotal + self.marginRAM,
            tileNames,
            macroHooks,
            self.marginFabCore,
            self.marginFab,
        ...
        for _subdir, _dirs, files in os.walk(
            f"{self.openlaneDir}/designs/{self.fabulousRunName}/src/BB"
        ):
            for file in files:
                src_path = Path(
                    f"{self.openlaneDir}/designs/{self.fabulousRunName}/src/BB/{file}"
                )
