...
Other than building the fabric with the `run_fab` command as shown in the {ref}`Quick start <quick-start>`, the user can
...
(fabulous-directory)=
### The .FABulous directory

When you create a project or run the FABulous flow, a hidden `.FABulous`
directory is created inside your project directory. This directory stores
critical metadata and generated artifacts that are required for bitstream
generation:

- `bitStreamSpec.csv` -- the bitstream specification in human readable form (generated by `gen_bitStream_spec`)
- `bitStreamSpec.bin` -- the bitstream specification (generated by `gen_bitStream_spec`)
- `bel.txt`, `pips.txt` and `bel.v2.txt` -- nextpnr architecture models (generated by `gen_model`)
- `.env` -- project-specific environment variables and settings
- `.fabulous_history` -- command history for the interactive shell

:::{warning}
**Do not delete the `.FABulous` directory.** Without it, you will not be
able to generate bitstreams for your fabric. If you are preparing a tapeout,
make sure to back up the entire project directory, including `.FABulous`,
so that you can regenerate bitstreams in the future.
:::

If you need to generate bitstreams for a fabric that was built with an older
version of FABulous, you can create a new project with `create-project`,
copy your fabric definition files (the `fabric.csv`, tile CSVs, and
primitives) into the new project, and re-run the FABulous flow to regenerate
the `.FABulous` directory contents.
...
```bash
FABulous> gen_switch_matrix LUT4AB RAM_IO
```
...
:::{note}
During model generation, the given file for `MATRIX` entry needs to be either a `.list` or `.csv` file.
:::

## Generate the configuration storage (RTL)

```bash
FABulous> gen_config_mem LUT4AB RAM_IO
```
...
```bash
FABulous> gen_tile LUT4AB RAM_IO
```
