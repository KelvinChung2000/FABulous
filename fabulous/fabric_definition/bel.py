...
from fabulous.fabric_definition.define import IO, HDLType, Status
...
class Bel:
    ...
    status : Status
        Support status set by the `EXPERIMENTAL` or `DEPRECATED` attribute.
        Defaults to `Status.STABLE`.
    ...
    status : Status
        Support status of the BEL. `EXPERIMENTAL` and `DEPRECATED` come from the
        module attribute of the same name. Default is `Status.STABLE`.
