...
def generateSuperTile(
        ...
        for y, row in enumerate(superTile.tileMap):
            for x, tile in enumerate(row):
                ...
                if (
                    ...
                    writer.addComment("CONFIG_PORT", onNewLine=False)
    # The supertile's tileMap carries its own origin, so the ladder offset is
    # scaled by that grid's north step, not the fabric's.
    dx, dy = user_clk_predecessor(user_clk_side, north_step=superTile.north_step)
    if not disable_user_clk:
    ...
    for y, row in enumerate(superTile.tileMap):
        for x, tile in enumerate(row):
            ...
            # consumes this clock is itself inside the supertile.
            if (
