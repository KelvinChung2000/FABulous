...
def _canonical_offset(
    ...
    """Derive a wire's grid offset under `origin` from its direction and reach.
...
def parse_port_line(
    ...
    if wire_direction in (
        ...
        x_offset, y_offset = _canonical_offset(
            wire_direction, x_offset, y_offset, line=line, origin=origin
        )
    ...
    for t in tilesData:
        ...
        for item in t:
            ...
            if temp[0] in Direction:
                port, common_wire_pair = parse_port_line(item, origin=origin)
                if "CARRY" in temp[6]:
                ...
                for line in iFile.split("\n"):
                    ...
                    port, common_wire_pair = parse_port_line(line, origin=origin)
                    ports.extend(port)
                    if common_wire_pair:
                        common_wire_pairs.append(common_wire_pair)
...
def parse_tile_from_dir(
    ...
    *,
    ...
    supertiles = parseSupertilesCSV(tile_csv, tile_dic, origin=origin)
...
def parseFabricCSV(fileName: str) -> Fabric:
    ...
    if origin is Origin.TOP_LEFT:
        logger.warning(
            ...
            "and drops this option. Set TopLeftOrigin,FALSE to migrate, then "
            "update anything that names a tile by coordinate, such as X0Y0."
        )

    # For backwards compatibility parse tiles in fabric config
    new_tiles, new_common_wire_pair = parseTilesCSV(
        fName, preserveListOrder, origin=origin
    )
    ...
    new_supertiles = parseSupertilesCSV(fName, tileDic, origin=origin)
    ...
    for i in parameters:
        ...
        if i[0].startswith("Tile"):
            ...
            new_tiles, new_common_wire_pair = parseTilesCSV(
                filePath.joinpath(i[1]), preserveListOrder, origin=origin
            )
            tileTypes += [new_tile.name for new_tile in new_tiles]
            tileDefs += new_tiles
            common_wire_pair += new_common_wire_pair
            ...
            new_supertiles = parseSupertilesCSV(
                filePath.joinpath(i[1]), tileDic, origin=origin
            )
            for new_supertile in new_supertiles:
                superTileDic[new_supertile.name] = new_supertile
