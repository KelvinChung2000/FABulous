...
def _safe_pformat(obj: object) -> str:
    ...
    The sink parses ``{...}`` as format fields and ``<...>`` as color markup;
    nested dict braces in an object repr overflow loguru's parser
    ("Max string recursion exceeded"). Doubling braces and escaping ``<`` makes
    the repr render literally.
    """
