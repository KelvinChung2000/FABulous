...
class N_IO_ports:
    def __init__(self):
        ...
        # bel ports
        self.N_from_fabric = MuxPort(BelPort(name='N_from_fabric', ioDirection=IO.INPUT, wireCount=1, isBus=True, prefix='N_', external=False), isBelPort=True, isBus=True, bitWidth=1)
        self.N_to_fabric = MuxPort(BelPort(name='N_to_fabric', ioDirection=IO.OUTPUT, wireCount=1, isBus=True, prefix='N_', external=False), isBelPort=True, isBus=True, bitWidth=1)
