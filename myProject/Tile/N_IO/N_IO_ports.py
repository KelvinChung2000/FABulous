...
class N_IO_ports:
    def __init__(self):
        ...
        # bel ports
        self.N_from_fabric = MuxPort(Port(name='from_fabric', ioDirection=IO.INPUT, wireCount=1, isBus=True), isBelPort=True, isBus=True, bitWidth=1)
        self.N_to_fabric = MuxPort(Port(name='to_fabric', ioDirection=IO.OUTPUT, wireCount=1, isBus=True), isBelPort=True, isBus=True, bitWidth=1)
