...
class N_IO_ports:
    def __init__(self, tilePorts: list[TilePort], belInputs: list[BelPort], belOutputs: list[BelPort]):
        # tile ports
        self.in2 = MuxPort(tilePorts[0], isTilePort=True, isBus=False, bitWidth=32)
        self.out2 = MuxPort(tilePorts[1], isTilePort=True, isBus=False, bitWidth=32)

        # bel ports
        self.N_from_fabric = MuxPort(belInputs[0], isBelPort=True, isBus=True, bitWidth=32)
        self.N_to_fabric = MuxPort(belOutputs[0], isBelPort=True, isBus=True, bitWidth=32)
