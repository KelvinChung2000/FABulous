...
class PE_ports:
    def __init__(self, tilePorts: list[TilePort], belInputs: list[BelPort], belOutputs: list[BelPort]):
        ...
        self.RES_en = MuxPort(belInputs[3], isBelPort=True, isBus=False, width=1)
        self.RES_reg_in = MuxPort(belInputs[4], isBelPort=True, isBus=True, width=32)
        self.RES_rst = MuxPort(belInputs[5], isBelPort=True, isBus=False, width=1)
        self.N_en = MuxPort(belInputs[6], isBelPort=True, isBus=False, width=1)
        self.N_reg_in = MuxPort(belInputs[7], isBelPort=True, isBus=True, width=32)
        self.N_rst = MuxPort(belInputs[8], isBelPort=True, isBus=False, width=1)
        self.E_en = MuxPort(belInputs[9], isBelPort=True, isBus=False, width=1)
        self.E_reg_in = MuxPort(belInputs[10], isBelPort=True, isBus=True, width=32)
        self.E_rst = MuxPort(belInputs[11], isBelPort=True, isBus=False, width=1)
        self.S_en = MuxPort(belInputs[12], isBelPort=True, isBus=False, width=1)
        self.S_reg_in = MuxPort(belInputs[13], isBelPort=True, isBus=True, width=32)
        self.S_rst = MuxPort(belInputs[14], isBelPort=True, isBus=False, width=1)
        self.W_en = MuxPort(belInputs[15], isBelPort=True, isBus=False, width=1)
        self.W_reg_in = MuxPort(belInputs[16], isBelPort=True, isBus=True, width=32)
        self.W_rst = MuxPort(belInputs[17], isBelPort=True, isBus=False, width=1)
        self.data_out = MuxPort(belOutputs[0], isBelPort=True, isBus=True, width=32)
        self.const_out = MuxPort(belOutputs[1], isBelPort=True, isBus=True, width=32)
        self.RES_reg_out = MuxPort(belOutputs[2], isBelPort=True, isBus=True, width=32)
        self.N_reg_out = MuxPort(belOutputs[3], isBelPort=True, isBus=True, width=32)
        self.E_reg_out = MuxPort(belOutputs[4], isBelPort=True, isBus=True, width=32)
        self.S_reg_out = MuxPort(belOutputs[5], isBelPort=True, isBus=True, width=32)
        self.W_reg_out = MuxPort(belOutputs[6], isBelPort=True, isBus=True, width=32)
