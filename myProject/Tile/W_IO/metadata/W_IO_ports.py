...
class W_IO_ports:
    def __init__(self):
        ...
        # bel ports
        self.W_from_fabric = MuxPort(BelPort(name='from_fabric', ioDirection=IO.INPUT, wireCount=32, isBus=True, prefix='W_', external=False, control=False), isBelPort=True, isBus=True, bitWidth=32)
        self.W_to_fabric = MuxPort(BelPort(name='to_fabric', ioDirection=IO.OUTPUT, wireCount=32, isBus=True, prefix='W_', external=False, control=False), isBelPort=True, isBus=True, bitWidth=32)
